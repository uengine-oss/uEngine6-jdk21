...
pids="$(lsof -ti tcp:9093 2>/dev/null || true)"
[ -n "$pids" ] && kill -9 $pids 2>/dev/null || true

export SPRING_PROFILES_ACTIVE=postgres,keycloak-installed
export DEFINITION_SERVICE_PORT=9093
...
export PROCESS_SERVICE_URL=http://localhost:9094

cd "$BACKEND_DIR/definition-service"
nohup mvn -Dmaven.repo.local="$HOME/.m2/repository" -DskipTests spring-boot:run >> "$LOG_DIR/definition-service.log" 2>&1 &
echo "definition-service starting on 9093, pid=$!, log=$LOG_DIR/definition-service.log"
