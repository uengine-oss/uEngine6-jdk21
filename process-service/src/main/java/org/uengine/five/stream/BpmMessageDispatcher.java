...
public class BpmMessageDispatcher {
    ...
    public void dispatch(Message<String> message) {
        ...
        String actorEndpoint = typeHeaderAsString(message.getHeaders().get("actorEndpoint"));
        ...
        if (typeHeader != null && !typeHeader.isEmpty()) {
            asyncEventListener.wheneverEvent(payload, typeHeader, corrKeyHeader, actorEndpoint);
