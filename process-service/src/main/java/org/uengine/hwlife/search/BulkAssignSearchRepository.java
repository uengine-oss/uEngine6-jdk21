...
public class BulkAssignSearchRepository {
  ...
  public SearchResult search(BulkAssignSearchRequest request) {
    ...
    dataQuery.select(worklist)
        .where(predicates(builder, dataQuery, worklist, instance, normalized))
    ...
    countQuery.select(builder.count(countWorklist))
        .where(predicates(builder, countQuery, countWorklist, countInstance, normalized));
